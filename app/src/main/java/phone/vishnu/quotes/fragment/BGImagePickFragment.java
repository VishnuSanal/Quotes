...
public class BGImagePickFragment extends BottomSheetDialogFragment implements BGImageRVAdapter.OnItemClickListener {
    ...
    private void loadPresentImages(View inflate) {
        ...
        RecyclerView.LayoutManager layoutManager = new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL) {
            ...
            public void onLayoutCompleted(RecyclerView.State state) {
                ...
                if (state.getItemCount() > 0 && MainActivity.bgDialog != null && MainActivity.bgDialog.isShowing())
        ...
        if (bgNameArrayList.size() == 0 && MainActivity.bgDialog != null && MainActivity.bgDialog.isShowing()) {
            MainActivity.bgDialog.dismiss();
        }
                ...
                for (StorageReference item : listResult.getItems())
                    if (!bgNameArrayList.contains(item.getName()))
                        item.getDownloadUrl().addOnSuccessListener(uri -> {
                            list.add(uri);
                            if (newAdapter != null && getContext() != null) {
